...
struct data_compare_center_less
{
    ...
    data_compare_center_less(Data<T> **data_array, int the_dim) :
            all_data(data_array), comp_dim(the_dim)
    {
    }
    bool operator()(const int id1, const int id2) const
    {
        ...
        if (val1 < val2)
            return true;
        else if ((val1 == val2) && (all_data[id1]->id < all_data[id2]->id))
            return true;
        else
            return false;
    }
...
// panos
	...
	bool dirty; // if has been modified
    Score textScore = 0;
    vector<RecordId>* verifiedRecords = new vector<RecordId> ;
...
public:
	...
	~RSTLeafNode();
	void flush(); //flush this to diskRSTNode
...
public:
...
#else
	tr1::unordered_map<int, RSTNode<Data, T> *> *node_buf; // provide a buffer mechanism
#endif

	//create a new tree or modify existing tree
	RStarTree(const char *fname, int dimension, int p_len, const char *path=NULL, PageFile::c_policy pol=PageFile::C_FULLMEM, int c_size=0, bool force_new=false);
    RStarTree(int dimension, int p_len, PageFile::c_policy pol = PageFile::C_FULLMEM, int c_size = 0);
    ...
    void bulkload_str(Data<T> **the_data_array, int num_data, double load_factor);
};
...
//RSTNode
template <template <typename TP> class Data, typename T>
RSTNode<Data, T>::RSTNode(RStarTree<Data, T> *rt) : the_tree(rt), dim(rt->dim), capacity(0), entry_num(0), page_id(-1), depth(-1), dirty(false){
}
...
//RSTNonLeafNode
template <template <typename TP> class Data, typename T>
RSTNonLeafNode<Data, T>::RSTNonLeafNode(RStarTree<Data, T> *rt) : RSTNode<Data, T>(rt)
{
	int page_len=rt->file->page_len;

	//compute header size and entry size
	int header_size=sizeof(bool)+sizeof(char)+sizeof(int);
	int entry_size=2*this->dim*sizeof(T)+sizeof(int)+sizeof(int);

	//compute capacity
	this->capacity=(page_len-header_size)/entry_size;
	if(this->the_tree->nonleaf_capacity<0) this->the_tree->nonleaf_capacity=this->capacity;
...
#else
		typename tr1::unordered_map<int, RSTNode<Data, T> *>::iterator iter=this->the_tree->node_buf->find(entry_page[sub_id]);
#endif		
...
{
	cout <<"[ depth:"<<this->depth << " id: " << this->id << " page_id: " << this->page_id;
...
RSTLeafNode<Data, T>::RSTLeafNode(RStarTree<Data, T> *rt) : RSTNode<Data, T>(rt)
{
	...
	int entry_size=Data<T>::get_size(this->dim);

	//compute capacity
	this->capacity=(page_len-header_size)/entry_size;
	if(this->the_tree->leaf_capacity<0) this->the_tree->leaf_capacity=this->capacity;
...
#else
	node_buf=new tr1::unordered_map<int, RSTNode<Data, T> *>();
#endif
...
#else
		for(typename tr1::unordered_map<int, RSTNode<Data, T> *>::iterator iter=node_buf->begin(); iter!=node_buf->end(); ++iter)
#endif
		{
			delete iter->second;
		}
...
#else
		typename tr1::unordered_map<int, RSTNode<Data, T> *>::iterator iter=node_buf->find(root_page);
#endif
...
#endif
        {
            iter->second->textScore = 0;
            iter->second->verifiedRecords = new vector<RecordId>;
            RSTNode<Data,T>* tmp = iter->second;
            if(tmp->is_leaf_node()){
                RSTLeafNode<TreeDataP, double>* leaf= static_cast<RSTLeafNode<TreeDataP, double> *>(tmp);
...
template<template<typename TP> class Data, typename T>
RStarTree<Data, T>::RStarTree(int dimension, int p_len, PageFile::c_policy pol, int c_size) :
        root(NULL), reinsrt_lvl(NULL), reinsrt_cands(NULL), leaf_capacity(-1), nonleaf_capacity(-1)
{
    //open file
    //ile = new PageFile(p_len, pol, c_size);
    header = new char[file->get_header_size()];
...
template<template<typename TP> class Data, typename T>
void RStarTree<Data, T>::bulkload_str(Data<T> **the_data_array, int num_data,
        double load_factor)
{
//    Timing tim;
...
//    rtree_sorting_time += (double)tim.stopClock() / CLOCKS_PER_SEC;
...
//    tim.startClock();
    ...
    {
        if (leaf_node == NULL)
            leaf_node = new RSTLeafNode<Data, T>(this);
        for (int j = blk_start; j < blk_end; j++)
        {
            if (leaf_node->insert(the_data_array[data_ids[j]], &temp)
                    != INSERT_NONE)
            {
                printf("Fatal error in file: %s, func: %s, line: %d\n",
                        __FILE__, __FUNCTION__, __LINE__);
        ...
        while (!nodes.empty())
        {
            ...
            if ((int) nodes.size() >= blk)
            {
                for (int i = 0; i < blk; i++)
                {
                    ...
                    nonleaf_node->enter(temp->get_mbr(the_mbr), temp->page_id,
                            temp->get_num_of_data(), temp);
                }
            }
            else
            {
