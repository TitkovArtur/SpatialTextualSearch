...
public:
	...
	static void Independent(float *data_f, int dim);
	static void Independent(double *data_d, int dim);
	static void Correlated(float *data_f, int dim, float sigma=defaultSigmaF);
	static void Correlated(double *data_d, int dim, double sigma=defaultSigmaD);
	static void AntiCorrelated(float *data_f, int dim, float sigma=defaultSigmaF);
	static void AntiCorrelated(double *data_d, int dim, double sigma=defaultSigmaD);
	static void Clustered(float *data_f, int dim, float **centers, int num_center, float sigma=defaultSigmaF);
	static void Clustered(double *data_d, int dim, double **centers, int num_center, double sigma=defaultSigmaD);
...
//split the range [val_min, val_max] into seg_num segments, given one value v
//every cell is like [cell_min, cell_max) except for the right-most cell to be [cell_min, cell_max]
//output is in range [0, seg_num-1]
template <typename T>
inline int GenericTool::DiscreteValueFloor(T val, int seg_num)
{
	return DiscreteValueFloor<T>(val, seg_num, rangeMin, rangeMax);
}

template <typename T>
inline int GenericTool::DiscreteValueFloor(T val, int seg_num, T val_min, T val_max)
{
	if((val<val_min)||(val>val_max)) return -1; //out of range

	T trans_val=(val-val_min)/(val_max-val_min);

	return min((int)floor(trans_val*(T)seg_num), seg_num-1);
}

//split the range [val_min, val_max] into seg_num segments, given one value v
//every cell is like (cell_min, cell_max] except for the left-most cell to be [cell_min, cell_max]
//output is in range [0, seg_num-1]
template <typename T>
inline int GenericTool::DiscreteValueCeil(T val, int seg_num)
{
	return DiscreteValueCeil<T>(val, seg_num, rangeMin, rangeMax);
}

template <typename T>
inline int GenericTool::DiscreteValueCeil(T val, int seg_num, T val_min, T val_max)
{
	if((val<val_min)||(val>val_max)) return -1; //out of range

	T trans_val=(val-val_min)/(val_max-val_min);

	return max((int)ceil(trans_val*(T)seg_num)-1, 0);
}

template <typename T>
inline T GenericTool::ContinuousValueFloor(int seg_id, int seg_num)
{
	return ContinuousValueFloor<T>(seg_id, seg_num, rangeMin, rangeMax);
}

template <typename T>
inline T GenericTool::ContinuousValueFloor(int seg_id, int seg_num, T val_min, T val_max)
{
	T trans_val=(T)seg_id/(T)seg_num;

	return val_min+(val_max-val_min)*trans_val;
}

template <typename T>
inline T GenericTool::ContinuousValueCeil(int seg_id, int seg_num)
{
	return ContinuousValueCeil<T>(seg_id, seg_num, rangeMin, rangeMax);
}

template <typename T>
inline T GenericTool::ContinuousValueCeil(int seg_id, int seg_num, T val_min, T val_max)
{
	T trans_val=(T)(seg_id+1)/(T)seg_num;
